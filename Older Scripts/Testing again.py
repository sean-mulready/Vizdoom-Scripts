...
# Step 1: Get a list of all .lmp files in the directory
...
def extract_block_and_episode(filename):
    ...
    block_part = parts[2]  # e.g., 'b1' from '1_Testsub1_b1_e1_basic_rec.lmp'
    episode_part = parts[3]  # e.g., 'e1' from '1_Testsub1_b1_e1_basic_rec.lmp'
...
for group_key, files in sorted_file_groups.items():
    ...
    print(f"Processing files for sub_num {group_key}:")
    game = vzd.DoomGame()
    for b in range(block_num):
        ...
        output_path = os.getcwd() + f'/data/sub-{group_key}'

        if not os.path.exists(output_path):
            os.makedirs(output_path)


        episode_filename = f"{output_path}/{group_key}_b_{b+1}_basic_game_dataframe.csv"
        with open(temp_file, 'w', newline='') as temp_csv: 
            ...
            for i in range(ep_basic):
                ...
                # tell to replay 
                game.replay_episode(f"{group_key}_Testsub{group_key}_b{b+1}_e{i+1}_basic_rec.lmp")
