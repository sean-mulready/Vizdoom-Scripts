...
pickle_file = "participants_data.pickle"
...
if os.path.exists(pickle_file) and os.path.getsize(pickle_file) > 0:
    with open(pickle_file, "rb") as infile:
        try:
            ...
            participants = []  # Handle the case where the file is empty but exists
else:
    participants = []  # Create a new list if the file doesn't exist or is empty

# Enter Subject Data
sub_num = '3'           # ongoing numerizing as string
sub_id = 'Testsub3'      # Construct ID as : First 2 letters of birth-location, first 2 letters of mother's first name, last two digits of birth-year, gender in one letter
age = 33                # Age as Integer
gender = 'nb'            # gender as string (m = male, f=female, nb = non-binary, o = other)
...
for b in range(block_num):
    ...
    for i in range(ep_basic):
        ...
        #start the recording
        game.new_episode(f"{sub_num}_{sub_id}_b{b+1}_e{i+1}_basic_rec.lmp")
...
# Save the updated list of participants back to the pickle file
with open(pickle_file, "wb") as outfile:
    pickle.dump(participants, outfile)


# pass on all variables needed for replaying, one for each participant


with open("myfile.pickle_{sub_num}", "wb") as outfile: 
