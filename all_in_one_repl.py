...
with open("myfile.pickle", "rb") as infile: 
    ...
    ep_health = pickle.load(infile)
    ep_home = pickle.load(infile)
    ep_cover = pickle.load(infile)
    block_num = pickle.load(infile)
    ticrate_basic = pickle.load(infile)
    ticrate_health = pickle.load(infile)
    ticrate_home = pickle.load(infile)
    ticrate_cover = pickle.load(infile)
...
######### REPLAY HEALTH GATHERING AND SAVING AS CSV ###################################################
#######################################################################################################
#######################################################################################################
...
episode_filename = f"{output_path}/{sub_num}_health_gathering_game_dataframe.csv"
with open(temp_file, 'w', newline='') as temp_csv: 
    csv_writer = csv.writer(temp_csv)
    columns = ["Sub_num",
                "Sub_ID",
    ...
    for i in range(ep_health):
        ...
        # tell to replay 
        game.replay_episode(f"{sub_id}_e{i+1}_health_gathering_rec.lmp")
        cumulative_reward = 0  # Initialize cumulative reward
        start_time = time.time()  # Record the start time
        ...
        while not game.is_episode_finished():
            ...
            cumulative_reward += reward  # Update cumulative reward
            current_time = game.get_episode_time()*(1/ticrate_health)
...
os.remove(temp_file)
for i in range(ep_health):
    os.remove(f"{sub_id}_e{i+1}_health_gathering_rec.lmp")



########### REPLAY MY WAY HOME AND SAVING AS CSV ################################################
#################################################################################################
#################################################################################################
...
with open(temp_file, 'w', newline='') as temp_csv: 
    ...
    for i in range(ep_home):
        ...
        while not game.is_episode_finished():
            ...
            cumulative_reward += reward  # Update cumulative reward
            current_time = game.get_episode_time()*(1/ticrate_home)
...
########## REPLAY TAKE COVER AND SAVING AS CSV ##########################################################################
#########################################################################################################################
#########################################################################################################################
...
episode_filename = f"{output_path}/{sub_num}_take_cover_game_dataframe.csv"
with open(temp_file, 'w', newline='') as temp_csv: 
    csv_writer = csv.writer(temp_csv)
    columns = ["Sub_num",
                "Sub_ID",
    ...
    for i in range(ep_cover):
        ...
        # tell to replay 
        game.replay_episode(f"{sub_id}_e{i+1}_take_cover_rec.lmp")
        cumulative_reward = 0  # Initialize cumulative reward
        start_time = time.time()  # Record the start time
        ...
        while not game.is_episode_finished():
            ...
            cumulative_reward += reward  # Update cumulative reward
            current_time = game.get_episode_time()*(1/ticrate_cover)
...
os.remove(temp_file)
for i in range(ep_cover):
    os.remove(f"{sub_id}_e{i+1}_take_cover_rec.lmp")
